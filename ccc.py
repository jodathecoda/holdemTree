...
class TreeNode:
    def __init__(self, master, name, details, depth=0, row=0):
        self.master = master
        self.name = name
        ...
        self.frame.grid(row=row, column=depth, sticky='w')
    ...
    def place_children(self, row_counter):
        current_row = row_counter[0]
        for child in self.children:
            child.row = current_row
            child.frame.grid(row=child.row, column=child.depth, sticky='w')
            current_row += 1
            row_counter[0] = current_row
            child.place_children(row_counter)

# Function to build the tree structure
def build_tree(master):
    root_node = TreeNode(master, "Flop", "Initial State", depth=0, row=0)
    ...
    # Recursively place children nodes
    root_node.place_children([1])  # Start placing children from row 1
